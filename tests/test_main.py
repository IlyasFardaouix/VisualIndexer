...
def test_run_pipeline():
    ...
    with patch("os.path.exists") as mock_exists, patch(
        "os.listdir"
    ) as mock_listdir, patch("builtins.print") as mock_print:
...
def test_main():
    ...
    with patch("argparse.ArgumentParser") as mock_argparse, patch(
        "your_module.run_pipeline"
    ) as mock_run_pipeline, patch("your_module.run_ui") as mock_run_ui, patch(
        "your_module.run_ingest_only"
    ) as mock_run_ingest_only:
...
def test_main_invalid_mode():
    ...
    with patch("argparse.ArgumentParser") as mock_argparse, patch(
        ...
        main()
        mock_print.assert_called_once_with(
            "Error: invalid choice: 'invalid' (choose from 'pipeline', 'ui', 'ingest')"
        )
...
def test_main_invalid_ingest():
    ...
    with patch("builtins.print") as mock_print, patch(
        "your_module.ImageIngestor.ingest_images"
    ) as mock_ingest_images:
...
def test_main_invalid_ocr():
    ...
    with patch("builtins.print") as mock_print, patch(
        "your_module.OcrProcessor.run_ocr"
    ) as mock_run_ocr:
...
def test_main_invalid_tagging():
    ...
    with patch("builtins.print") as mock_print, patch(
        "your_module.ClipTagger.get_clip_tags"
    ) as mock_get_clip_tags:
...
def test_main_invalid_embeddings():
    ...
    with patch("builtins.print") as mock_print, patch(
        "your_module.EmbeddingManager.generate_embedding"
...
def test_main_invalid_save_embeddings():
    ...
    with patch("builtins.print") as mock_print, patch(
        "your_module.EmbeddingManager.store_embeddings"
    ) as mock_store_embeddings:
...
def test_main_invalid_save_ocr():
    ...
    with patch("builtins.print") as mock_print, patch(
        "your_module.OcrProcessor.save_ocr_results"
    ) as mock_save_ocr_results:
